...
export default function AuthScreen({ onAuthenticated }) {
  ...
  const validateUsername = (username) => {
    ...
    if (/script|javascript|onerror|onclick|onload/i.test(username)) {
      return { valid: false, error: "Username contains invalid characters" };
    }

    // Check for SQL injection patterns
    if (
      /('|(--|;|\/\*|\*\/|xp_|sp_|exec|execute|select|insert|update|delete|drop|create|alter))/i.test(
        username,
      )
    ) {
      return { valid: false, error: "Username contains invalid characters" };
    ...
    if (/%[0-9a-f]{2}/i.test(username)) {
      return {
        valid: false,
        error: "Username cannot contain encoded characters",
      };
    }
  ...
  const getTitle = () => {
    switch (mode) {
      case "choice":
        return t("auth_welcome");
      case "register_name":
        return t("auth_create_account");
      case "register_password":
        return t("auth_create_account");
      case "create_drawing":
      case "confirm_drawing":
        return t("auth_first_time_title");
      case "login":
        return `${t("auth_welcome_back")}, ${savedUsername || ""}`;
      case "import_file":
        return t("auth_import_account");
      case "import_drawing":
      case "import_confirm_drawing":
        return t("auth_first_time_title");
      default:
  ...
  const getSubtitle = () => {
    switch (mode) {
      case "choice":
        return t("auth_your_impact");
      case "register_name":
        return t("auth_enter_username");
      case "register_password":
        return t("auth_create_password");
      case "create_drawing":
        return t("auth_first_time_subtitle");
      case "confirm_drawing":
        return t("auth_confirm_drawing");
      case "login":
        return t("auth_login_title");
      case "import_file":
        return t("auth_import_desc");
      case "import_drawing":
        return t("auth_first_time_subtitle");
      case "import_confirm_drawing":
        return t("auth_confirm_drawing");
              ...
              <GlassCard variant="elevated" style={styles.choiceCard}>
                ...
                <View style={styles.choiceContent}>
                  ...
                  <Text
                    style={[
                      styles.choiceTitle,
                      { color: isDark ? "#fff" : BRAND.oceanDeep },
                    ]}
                  >
          ...
          >
            <GlassCard variant="default" style={styles.formCard}>
              ...
              <TextInput
                ...
                value={regUsername}
                onChangeText={(text) => {
                  // Filter out dangerous characters in real-time
              ...
              >
                ...
                <Text
                  style={[
                    ...
                    { color: importFileName ? colors.text : colors.textMuted },
                  ]}
                  numberOfLines={1}
                >
            ...
            >
              {(mode === "create_drawing" || mode === "confirm_drawing") && (
                <View
                  style={{
                    alignItems: "center",
                    paddingVertical: winH < 700 ? rs(4) : rs(10),
                  }}
                >
              ...
              <View style={styles.drawingArea}>
                ...
                <Text
                  style={[
                    styles.pinHint,
                    { color: colors.textMuted, marginTop: rs(8) },
                  ]}
                >
                  {statusText}
                </Text>
